...
貼り付け先に同名項目がある場合は競合ダイアログを表示し、MVP では選択した方針を残りの競合にも一括適用します。削除は OS のゴミ箱移動で実行し、複数対象時のみ確認ダイアログを挟みます。rename は単一対象に限定し、新規作成とあわせて画面下部の入力バーで値を編集します。通常の 1 行ヘルプには `s` / `d` / `F2` / `Ctrl+N` / `Ctrl+Shift+N` を表示します。ステータスバーのソート表示は `name asc dirs:on` の形式で、ディレクトリ優先状態も含めます。ソート変更後は同じ項目が可視である限りカーソル位置を維持します。バリデーションや重複名エラー時は入力値を保持したままエラー通知を表示します。
