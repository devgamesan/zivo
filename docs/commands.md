...
| `Change permissions` | Exactly one target is selected or focused in a real filesystem workspace | Starts permission input for a single target. Enter a three-digit octal mode such as `755` or `644`. This command is hidden in search workspaces. |
